...
class Game:
    ...
    def needs_bot_turn(self) -> bool:
        """Retourne True si le joueur courant est un bot et doit jouer."""
        return self.get_current_player().is_bot

    def _play_bot_turn(self) -> None:
        """Joue automatiquement un tour complet pour le bot courant.
        Ne s'appelle pas récursivement — c'est la couche route qui reboucle
        via une greenlet après chaque broadcast."""
        bot = self.get_current_player()
        print(f"[BOT] Tour du bot {bot.name}")

        # Phase PIOCHE : skip si nécessaire
        if bot.skip_turn > 0:
            bot.skip_turn -= 1
            print(f"[BOT] {bot.name} passe son tour (skip_turn)")
            self.add_to_history(f"🤖 {bot.name} passe son tour")
            self.player_turn = (self.player_turn + 1) % len(self.players)
            self.turn_state = TurnState.PIOCHE
            return
        ...
        # Préférer la défausse si la carte est jouable
        last_discard = self.get_last_discard()
        if last_discard:
            ok, _ = last_discard.can_be_played(bot, self)
            if ok:
                card = self.take_card_from_discard()
                if card:
                    bot.add_card_to_hand(card)
                    self.turn_state = TurnState.POSE
                    card.play_card(self, bot, interface)
                    self.add_to_history(f"🤖 {bot.name} a joué la carte de la défausse {card.get_name()}")
                    self.player_turn = (self.player_turn + 1) % len(self.players)
                    self.turn_state = TurnState.PIOCHE
                    return
        ...
        # Phase POSE : jouer une carte valide ou se défausser
        playable = [c for c in bot.hand if c.can_be_played(bot, self)[0]]
        if playable:
            chosen = interface.ask_card("", playable, None)  # type: ignore[arg-type]
            if chosen and chosen.can_be_played(bot, self)[0]:
                chosen.play_card(self, bot, interface)
                self.add_to_history(f"🤖 {bot.name} a joué {chosen.get_name()}")
                self.player_turn = (self.player_turn + 1) % len(self.players)
                self.turn_state = TurnState.PIOCHE
                return
        ...
        if bot.hand:
            to_discard = interface.ask_card("", bot.hand, None)  # type: ignore[arg-type]
            if to_discard:
                bot.remove_card_from_hand(to_discard)
                self.discard.append(to_discard)
                self.add_to_history(f"🤖 {bot.name} a défaussé {to_discard.get_name()}")
        self.player_turn = (self.player_turn + 1) % len(self.players)
        self.turn_state = TurnState.PIOCHE

    def add_bot(self, name: str | None = None) -> "Player":
        """Crée et retourne un nouveau joueur bot (sans l'ajouter à la partie en cours).
        Destiné à être utilisé au moment de la création de la partie (avant __init__)."""
        raise NotImplementedError(
            "add_bot() sert de référence. Instanciez Player(name, id, BotIO(), is_bot=True) "
            "dans votre logique de lobby avant de passer la liste au constructeur Game()."
        )
